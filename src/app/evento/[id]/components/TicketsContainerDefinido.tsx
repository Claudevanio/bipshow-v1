...
export function TicketsContainerDefinido ({
  ...
  const eventosIds = queryParams.get('eventosIds')
 
  const {
    ticket, eventTicket, ticketsPurchase, chairs, rank, 
  } = useEventTicket();
  
  const { handleClearTicket, ticketFormatted, handleSelectTicket, quantity, quantityTickets, setIsQuantity, handleSelectSector, handleEditChair, handleShowModal } = useEventTicket();
  ...
  const {
    nomeSector, selectedChairs, handleSeletedRemoved, handleClearSector, handleSelectTicketWithChairs, idSector, colorSector, multipleEventTicket
  } = useEventTicket();
  ...
  return (  
    ...
    >
      ...
      >
        ...
        >
          {
            (multipleEventTicket && multipleEventTicket?.length > 0) ? multipleEventTicket.map((item, index) => <li
              key={index}
              className='flex pb-3'
              onClick={
                () => {
                  router.push(`/evento/${item.id}${eventosIds ? `?eventosIds=${eventosIds}` : ''}`)
                }
              }
            >
              {
              ...
              <div
                className={`flex items-center gap-1 text-textPrimary px-3 py-1 rounded-lg hover:bg-gray cursor-pointer ${item.id === eventTicket?.id ? ' bg-gray !text-softBlue' : 'font-normal'}`}
              >
                <p
                  className='font-bold text-3xl'
                >
                  {
                    dayjs(item?.dataRealizacao).format('DD')
                  }
                </p>
                <div
                ...
                >
                  <p>
                    {
                      dayjs(item?.dataRealizacao).format('MMM').toUpperCase()
                    }
                  </p>
                  <p
                  ...
                  >
                    {
                      dayjs(item?.dataRealizacao).format('dddd').replace('-feira', '')
                    }
                  </p>
                </div>
              </div>
          ...
          >
            <div
              className={`flex items-center gap-1 text-textPrimary px-3 py-1 rounded-lg hover:bg-gray cursor-pointer ${true ? ' bg-gray !text-softBlue' : 'font-normal'}`}
            >
              <p
                className='font-bold text-3xl'
              >
                {
                  dayjs(eventTicket?.dataRealizacao).format('DD')
                }
              </p>
              <div
                className='flex flex-col gap-0 '
              >
                <p>
                  {
                    dayjs(eventTicket?.dataRealizacao).format('MMM').toUpperCase()
                  }
                </p>
                <p
                  className='text-[9.6px] first-letter:uppercase'
                >
                  {
                    dayjs(eventTicket?.dataRealizacao).format('dddd').replace('-feira', '')
                  }
                </p>
              </div>
            </div>
        ...
        >
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              selectedItem.typeTicket = eventType
